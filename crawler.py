...
def event_regex(value: str) -> str:
    ...
    if normalized not in EVENT_REGEXES:
        supported = ", ".join(sorted({"race", "sprint", "qualifying", "practice", "sprint-qualifying", "paddock", "press"}))
...
def is_pagination_link(link: LinkItem, source_url: str) -> bool:
    ...
    if not (title.isdigit() or title in {"next", "previous", "prev"} or link.title.strip() in {"»", "«", "›", "‹"}):
        return False
...
def crawl_links(config: WatchConfig) -> list[LinkItem]:
    ...
    while pages_to_fetch and len(fetched_pages) < max(1, config.max_pages):
        ...
        for link in links:
            ...
            if is_pagination_link(link, page_url) and link.url not in fetched_pages and link.url not in pages_to_fetch:
...
def item_matches(item: LinkItem, config: WatchConfig) -> bool:
    ...
    if config.regex and not re.search(config.regex, f"{item.title} {item.url}", re.IGNORECASE):
        return False
...
def check_once(config: AppConfig) -> int:
    ...
    if config.watch.include_seen:
        ...
        print(f"Checked {config.watch.url}: visited {len(matches)} match(es), including already seen matches.")
        return len(matches)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--notify-existing", action="store_true", help="Alert for matches found on first run.")
    parser.add_argument(
        "--include-seen",
        action="store_true",
