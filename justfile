...
lint:
    uv run python -B -m py_compile src/grandprix_watcher/*.py tests/*.py
...
clean:
    uv run python -c 'from pathlib import Path; import shutil; paths = [".coverage", "coverage.xml", "build", "dist", "site", "htmlcov", ".pytest_cache", ".ruff_cache", ".mypy_cache", "src/grandprix_watcher.egg-info", "__pycache__", "tests/__pycache__", "src/grandprix_watcher/__pycache__"]; [shutil.rmtree(path, ignore_errors=True) if Path(path).is_dir() else Path(path).unlink(missing_ok=True) for path in paths]'

# Run one normal check using config.toml.
check config=config:
    uv run grandprix-watcher --config {{config}}

# Keep polling using config.toml.
watch config=config:
    uv run grandprix-watcher --config {{config}} --watch

# Run one check and notify/open matches even if they were already seen.
force config=config:
    uv run grandprix-watcher --config {{config}} --include-seen

# Run one check for an event and Grand Prix location.
for event place config=config:
    uv run grandprix-watcher --config {{quote(config)}} --grand-prix {{quote(place)}} --event {{quote(event)}}

# Keep polling for an event and Grand Prix location.
watch-for event place config=config:
    uv run grandprix-watcher --config {{quote(config)}} --grand-prix {{quote(place)}} --event {{quote(event)}} --watch

# Run one event/location check and notify/open even if the match was already seen.
force-for event place config=config:
    uv run grandprix-watcher --config {{quote(config)}} --grand-prix {{quote(place)}} --event {{quote(event)}} --include-seen
